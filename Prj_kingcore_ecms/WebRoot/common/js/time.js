...
//function YYMMDD() {    return('<span class="rilia">'+SY+'</span>年<span class="rilia">'+(SM+1)+'</span>月<span class="rilia">'+SD+'日')}
function weekday() {
    var cl = '';
...
function clock(){
	now=new Date();
	SY = now.getFullYear(); SM = now.getMonth(); SD = now.getDate();
	return YYMMDD() + ' ' + weekday(); // + ' ' + solarDay()  //by wzw
}